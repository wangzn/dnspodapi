	...
	if ret, ok := res.Data.(*DomainCreateResult); ok {
		if ret != nil {
			if ret.Status.Code != "1" {
				return nil,
					Err(ErrInvalidStatus, "Domain.Create", ret.Status.Code,
						ret.Status.Message)
			}
			id := ret.Domain.ID
			if id != "" {
...
func GetDomainInfo(name string) (*DomainEntry, error) {
	...
	if ret, ok := res.Data.(*DomainInfoResult); ok {
		if ret != nil {
			if ret.Status.Code != "1" {
				return nil,
					Err(ErrInvalidStatus, "Domain.Info", ret.Status.Code,
						ret.Status.Message)
			}
...
func GetDomainList() ([]DomainEntryIDInt, error) {
	...
	if ret, ok := res.Data.(*DomainListResult); ok {
		if ret != nil {
			if ret.Status.Code != "1" {
				return nil,
					Err(ErrInvalidStatus, "Domain.List", ret.Status.Code,
						ret.Status.Message)
			}
...
func RemoveDomain(name string) (bool, error) {
	...
	if ret, ok := res.Data.(*DomainRemoveResult); ok {
		if ret != nil {
			...
			return false,
				Err(ErrInvalidStatus, "Domain.Remove", ret.Status.Code, ret.Status.Message)
		}
	}
...
func SetDomainStatus(name string, status string) error {
	...
	if ret, ok := res.Data.(*DomainStatusResult); ok {
		if ret != nil {
			...
			return Err(ErrInvalidStatus, "Domain.Status", ret.Status.Code,
				ret.Status.Message)
		}
	}
